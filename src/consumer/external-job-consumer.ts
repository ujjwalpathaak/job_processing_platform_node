...
export class ExternalJobConsumer extends AbstractJobConsumer {
  ...
  public async start(): Promise<void> {
    ...
    await channel.consume(Queue.EXTERNAL, async (msg) => {
      ...
      try {
        Logger.info(
          `job | consumer message received | consumer=${this.consumerName} | queue=${Queue.EXTERNAL} | payload=${msg.content.toString()}`,
        );
        ...
        Logger.error(
          `job | consumer message failed | consumer=${this.consumerName} | queue=${Queue.EXTERNAL} | error=${error}`,
        );
